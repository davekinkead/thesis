...
var Agent, Space, agents, canvas, d3, height, move, populate, run, running, simulation, width;

Agent = (function() {
  function Agent(race, tolerance, depth) {
    this.race = race;
...
Agent = (function() {
  function Agent(space1) {
    ...
    })(), this.race = ref[0], this.xenophobia = ref[1];
  ...
  Agent.prototype.isHappy = function() {
    var homogeneity;
    homogeneity = this.space.homogeneity(this);
    if (homogeneity >= this.xenophobia) {
      return true;
    } else {
      return false;
    }
...
Space.prototype.neighbourhood = function(x, y) {
  var i, len, neighbours, other, ref, ref1, ref2;
  neighbours = [];
  ref = this.agents;
  for (i = 0, len = ref.length; i < len; i++) {
    other = ref[i];
    if ((x - other.depth < (ref1 = other.x) && ref1 < x + other.depth) && (y - other.depth < (ref2 = other.y) && ref2 < y + other.depth)) {
...
Space.prototype.homogeneity = function(agent) {
  var i, len, neighbour, neighbours, others, same;
  same = 0;
  others = 0;
  neighbours = this.neighbourhood(agent.x, agent.y);
  for (i = 0, len = neighbours.length; i < len; i++) {
    neighbour = neighbours[i];
    if (agent.race === neighbour.race) {
      same += 1;
    }
    if (agent.race !== neighbour.race) {
      others += 1;
